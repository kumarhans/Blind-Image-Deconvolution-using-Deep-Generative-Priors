...
SAVE_PATH       = './results/SVHN/deblurring_video_alg1_init/deblurring_'
# -----------------------------------------------------------------------
...
prev = blur_decoder.input

for i in range(len(Blurry_Images_orig)):
    print(i)
    image_gradients, blur_gradients, get_loss, prev = Generate_Gradient_Functions2(rr = 10,
                                                                        ...
                                                                        channels = CHANNELS, blur_decoder_last_input=prev)
    
    m_hat_i, h_hat_i, Loss_i = Optimize_Parallel2(blurry_fourier = Blurry_Images[i*RANDOM_RESTARTS:(i+1)*RANDOM_RESTARTS], stepsize=step_size,steps = STEPS,
                                        image_grad = image_gradients , blur_grad = blur_gradients, 
                                        getloss = get_loss, latent_image_dim = svhn_latent_dim , latent_blur_dim = blur_latent_dim, initial_h = initial_h)
...
np.save('./results/Loss_vs_iter_init.npy', Loss)

Loss = np.mean(Loss, axis = 0) 
plt.plot(Loss)
plt.ylabel("Loss")
plt.xlabel("Iteration")
plt.savefig('./results/'+"Loss_vs_iter_init.jpg")

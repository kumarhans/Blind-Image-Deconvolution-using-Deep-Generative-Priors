...
import ipdb
# paths
Orig_Path       = './results/hansSVHN/BlurSeq/blur02.png'
...
SAVE_PATH       = './results/SVHN/hans_data/'
# -----------------------------------------------------------------------
...
for i in tqdm(range(len(X_Orig)), ascii=True, desc ='Gen-Test-Blurry'):
    ...
    ipdb.set_trace()
    h,w=W[i].shape
    w_np = np.zeros(W[i].shape)
    w_np[int(h/2),int(w/2)]=1
...
##replacing image
# blurry_image=np.load('./results/hansSVHN/BlurSeq/blur02.png')
# Blurry_Images = np.array(Blurry_Images)

print(Blurry_Images.shape)
